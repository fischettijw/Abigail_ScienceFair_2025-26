...
VACCINATED_PERCENT = 0.2     # Starting vaccinated proportion (adjustable)
...
DAYS_TO_DEATH = 10  # Days before an infected person has a chance to die
...
class Person:
    ...
    def draw(self):
        color = BLUE if self.status == "susceptible" else GREEN if self.status == "immune" else RED if self.status == "infected" else GRAY
...
while running:
    ...
    deceased_count = sum(1 for p in people if p.status == "deceased")
    susceptible_count = POPULATION - infected_count - immune_count - deceased_count

    # Display stats
    font = pygame.font.SysFont(None, 30)
    stats = f"Day: {days} | Infected: {infected_count} | Immune: {immune_count} | Susceptible: {susceptible_count} | Deceased: {deceased_count}"
    ...
    pygame.draw.circle(screen, GRAY, (10, 110), 5)
    screen.blit(font.render("Deceased", True, BLACK), (20, 105))
    ...
    screen.blit(progress_display, (10, 130))
    ...
    if not herd_immunity_achieved and immunity_progress >= HERD_IMMUNITY_THRESHOLD:
        ...
        screen.blit(herd_result, (10, 160))
